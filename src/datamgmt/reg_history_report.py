...
class RegHistoryReport:
  ...
  def generate(self, path):
    log_info(f"reg_history_report: Generating report at {path}")
    history = self.load_history()  
    self.update_google_sheet(history)
  ...
  def update_google_sheet(self, history):
    all_data = history | self.current_year_data(history["date_labels"])
    sheet_data = [["Year"] + history["date_labels"]]
